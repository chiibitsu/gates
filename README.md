...
A release's `CITATION.cff` names its own version — that is the part that must be right, and from v1.0.4 it is. The workflow pins *inside* a release necessarily reference the **previous** release, because a commit cannot contain its own future SHA; they are moved forward in the next release commit. Take the SHA to pin from this table, not from the example in a checkout.

| Version | Commit | Use it? |
|---|---|---|
| **v1.0.4** | the merged head of #4 | **Yes — use this one.** The first release whose own `CITATION.cff` names the version on its tag. Gates byte-identical to v1.0.2 and v1.0.3, so it carries their one known false green, described below. |
